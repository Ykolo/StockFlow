    ...
    <Form {...form}>
      <form className="flex flex-col text-marine gap-4" onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <FormItem>
            ...
            <Popover>
              <PopoverTrigger asChild>
                <FormControl>
                  <Button variant={"outline"} role="combobox" className={cn("justify-between bg-amande hover:bg-amande-hover ", !field.value && "text-marine/50")}>
                    {field.value ? companies.find((company: companyType) => company.id === field.value)?.name : "Sélectionner une entreprise"}
              ...
              <PopoverContent className="bg-amande">
                <Command className="bg-amande text-marine">
                  <CommandInput placeholder="Dans quelle entreprise travaillez vous ?" className="placeholder:text-marine" />
                  <CommandList>
                    <CommandEmpty>Aucune entreprise trouvé</CommandEmpty>
                    <CommandGroup>
                      {companies?.map((company: companyType) => (
                        <CommandItem value={company.name} key={company.id} onSelect={()=>{form.setValue("companyId",company.id)}} className="text-marine active:bg-amande hover:bg-amande-hover data-[selected=true]:bg-amande-hover data-[selected=true]:text-marine" >
                          {company.name}
                          <Check className={cn("ml-auto", company.name === field.value ? "opacity-100" : "opacity-0")} />
                        </CommandItem>
                      ))}
                    </CommandGroup>
                  </CommandList>
        ...
        <Button 
          ...
          type="submit" 
          disabled={isLoading}
          onClick={() => console.log("Button clicked, loading state:", isLoading)}
        >
